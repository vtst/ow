...
public class AbstractValidatorPropertyPage {
   ...
   */
  public boolean init(IProject project, Shell shell, IStore store) {
    ...
    store = getStore(inspector);
    return true;
  }

  private IStore getStore(DeclarativeValidatorInspector inspector) {
    return this.store;
  }
  ...
  private void fillList() throws CoreException {
    for (Group group : inspector.getGroups()) {
      ...
      item.setChecked(store.getEnabled(group));
    }
    setCheckbox(store.getCustomized());
  }
  ...
  private void showErrorMessage(String message, IStatus status) {
    ErrorDialog.openError(this.shell, this.getClass().getName(), message, status); 
  }
  
  private void showErrorMessage(String message, String cause) {
    ErrorDialog.openError(
        this.shell, this.getClass().getName(), message, 
  ...
  public boolean performOk() {
    ...
    try {
      if (checkbox.getSelection()) {
        ...
        for (int i = 0; i < inspector.getGroups().size(); ++i) {
          store.setEnabled(inspector.getGroups().get(i), list.getItem(i).getChecked());
        }
      } else {
